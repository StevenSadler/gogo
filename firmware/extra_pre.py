...
if is_clean:
    ...
    IDENTITY_PATH = PROJECT_DIR / "config" / "generated" / "firmware_identity.json"
    ...
    runtime_contract_hash = contract_hash[:8]
    ...
    runtime_build_hash = build_hash[:8]
    ...
    # ----------------------------------------------------------------------
    # Write build_metadata.h
    # ----------------------------------------------------------------------
    build_timestamp_utc = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC")
    header_path = GENERATED_HEADER_DIR / "build_metadata.h"
    ...
    IDENTITY_PATH.write_text(
        json.dumps(firmware_identity, indent=2) + "\n"
    )

    log(f"Wrote firmware identity: {IDENTITY_PATH}")
